	...
	public async override UniTask BeginTurn(Board board, int color)
	{
		...
		foreach (Node node in myNodes)
		{
			...
			foreach (List<Node> path in possiblePaths)
			{
				...
				// 2. Recurse using Maximax
				float score = Maximax(board, color, m_maxDepth - 1);
	...
	private float Maximax(Board board, int color, int depth)
	{
		...
		foreach (Node node in myNodes)
		{
			...
			foreach (List<Node> path in paths)
			{
				...
				// Recurse (In Maximax, we always seek the MAX, even at the next depth)
				float score = Maximax(board, color, depth - 1);
